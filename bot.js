...
RoleGuard.on("ready", async () => {
    RoleGuard.user.setStatus("iddle");
...
RoleGuard.on("message", async (message) => {
    ...
    if ((Config.BotOwner.includes(message.author.id) || veri.Full.some(us => message.author.id == us || message.member.roles.cache.has(us))) && command == "!güvenli") {
...
${veri.ChatG.length > 0 ? veri.ChatG.map(x => `${message.guild.members.cache.get(x) ? message.guild.members.cache.get(x) : message.guild.roles.cache.get(x) ? message.guild.roles.cache.get(x) : message.guild.channels.cache.get(x)}`).join("\n") : "Herhangi bir üye & rol güvenliye eklenmedi!"}`)
    ...
    if (Config.BotOwner.includes(message.author.id) && command == "!dagitici") {
        ...
        let roles = await RoleModel.findOne({
            ...
            let newRole = message.guild.roles.cache.find(e => e.name == roles.name);
            ...
            await veriDegis(args[1], newRole.id);
	let kanalPermVeri = roles.channelOverwrites;
            ...
            clients.forEach((_guardClient, _index) => {
                let membersId = roles.members.splice(0, (_index == 0 ? perUser + countUser : perUser));
	...
	if (Config.BotOwner.includes(message.author.id) && command == "!kizin") {
...
ChannelGuard.on("ready", () => {
    ChannelGuard.user.setStatus("iddle");
...
OtherGuard.on("ready", () => {
    OtherGuard.user.setStatus("iddle");
...
/*
let reklamLimit = {};
OtherGuard.on("message", async (message) => {
    if (message.author.bot || message.channel.type == "dm") return;
    if (await checkPermission(OtherGuard, message.author.id, "chatguard") || message.author.id === message.guild.owner.id) return;
    let messages = message.channel.messages.cache.array();
    ...
    if (ms.length > 5 && !["game","owo","fun","gif","t-sustum"].some(ver => message.channel.name.includes(ver))) {
        ms.forEach(m => m.delete().catch(() => {}));
        message.member.roles.add(Config.Mute);
        message.reply("Spam yaptığın için sunucumuzda 3 dakika boyunca susturuldun!");
        setTimeout(() => {
            message.member.roles.remove(Config.Mute);
        }, 1000 * 60 * 3);
        return;
    }
    ...
    if (kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase())) && !["t-sustum"].some(ver => message.channel.name.includes(ver))) {
		...
		message.delete().then(msg => {
          message.reply(" etme eyleme kardeşim günah ya.").then(m => m.delete({
            timeout: 2000
          })).catch(err => {});
        });
...
OtherGuard.on("messageUpdate", async (oldMessage, newMessage) => {
    ...
    if (await checkPermission(OtherGuard, oldMessage.author.id, "chatguard") || oldMessage.author.id === oldMessage.guild.owner.id) return;
    const kelime = ["discord.gg", "discord.me", "discordapp.com", "discord.io", "discord.tk", ".gg/", ".gg"];
    ...
    if (kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase())) && !["sus"].some(ver => newMessage.channel.name.includes(ver))) {
      newMessage.delete().then(message => {
        newMessage.reply(" etme eyleme kardeşim günah ya.").then(m => m.delete({
          timeout: 2000
        })).catch(err => {});
      });
    }
    ...
    if (newMessage.mentions.members.size >= Config.Limit.Etiket || newMessage.mentions.roles.size >= Config.Limit.Etiket || newMessage.mentions.channels.size >= Config.Limit.Etiket) return newMessage.delete().catch(() => {});
});
*/
const logs = require('discord-logs');
const safeMembers = require("./models/safeMembers");
logs(OtherGuard);

let baglantiKesmeLimit = {};
let susturmaLimit = {};
/*
OtherGuard.on("voiceChannelLeave", async (member, channel) => {
    let logs = await member.guild.fetchAuditLogs({
        limit: 1,
    ...
    if (!baglantiKesmeLimit[entry.executor.id]) baglantiKesmeLimit[entry.executor.id] = 1;
    if (baglantiKesmeLimit[entry.executor.id] && baglantiKesmeLimit[entry.executor.id] >= Config.Limit.BaglantiKesme) {
        ...
        sendLog(`**${entry.executor.tag}** (${entry.executor.id}) adlı yetkili bağlantı kesme sınırını aştığı için jail'e gönderildi!`);
    };
    baglantiKesmeLimit[entry.executor.id] += 1;
    setTimeout(() => {
        baglantiKesmeLimit[entry.executor.id] = 1;
